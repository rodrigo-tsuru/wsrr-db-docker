...
SET CURRENT SCHEMA WSRR /
---------------------------------------------------------------------
---- Stored procedures for encoding strings and URI's
---------------------------------------------------------------------
CREATE PROCEDURE WSRR.ENCSUBJ(
...
CREATE PROCEDURE WSRR.ENCGRAPH(
    IN str_in   VARCHAR(1024),
    IN hash_in  BIGINT)
    LANGUAGE SQL MODIFIES SQL DATA
CRSUBJ: BEGIN
...
CRTSTMT: BEGIN
    ...
    CALL WSRR.ENCGRAPH(graph_uri_in, graph_hash_in); 
...
CRTSTMT: BEGIN
    ...
    IF subj_uri_in IS NOT NULL THEN
        CALL  WSRR.ENCSUBJ(subj_uri_in, subj_hash_in); 
    END IF;
   
    CALL WSRR.ENCPRED(pred_uri_in, pred_hash_in);
    
    IF obj_in IS NOT NULL THEN
        CALL WSRR.ENCOBJ(obj_in, obj_hash_in, obj_lang_in, obj_district_in, obj_variant_in);
    END IF;
    
    IF graph_hash_in IS NULL THEN
       CALL  WSRR.NGCRTST(subj_hash_in, pred_hash_in, obj_hash_in, set_obj_hash_rel_in, obj_typ_in);
    ELSE
       CALL  WSRR.GCRTST(subj_hash_in, pred_hash_in, obj_hash_in, obj_lang_in, obj_typ_in, graph_uri_in, graph_hash_in);
    END IF;
END CRTSTMT/
...
UPDSTMT: BEGIN
    ...
    IF new_obj_in IS NOT NULL THEN
        CALL WSRR.ENCOBJ(new_obj_in, new_obj_hash_in, new_obj_lang_in, new_obj_district_in, new_obj_variant_in);
    END IF;
    ...
    IF (graph_hash_in IS NULL) THEN 
        CALL WSRR.NGUPDST(subj_hash_in, pred_hash_in, obj_hash_in, obj_typ_in, alt_obj_typ, new_obj_hash_in, set_obj_hash_rel_in, new_obj_typ_in);
    ELSE
        CALL WSRR.GUPDST(subj_hash_in, pred_hash_in, obj_hash_in, obj_lang_in, obj_typ_in, alt_obj_typ, graph_hash_in, new_obj_hash_in, new_obj_lang_in, new_obj_typ_in);
        
    END IF;

END UPDSTMT/

GRANT EXECUTE ON PROCEDURE WSRR.ENCSUBJ TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.ENCGRAPH TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.ENCOBJ TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.NGCRTST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.GCRTST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.CRTST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.NGUPDST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.GUPDST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.UPDST TO USER WSRRUSER /
GRANT EXECUTE ON PROCEDURE WSRR.DELST TO USER WSRRUSER /
